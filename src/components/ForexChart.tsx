...
import { ComposedChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Bar, Rectangle, Brush, ReferenceLine } from "recharts";
...
const CustomCandlestick = (props: any) => {
  ...
  return <g>
      <line x1={x + width / 2} y1={topWickY} x2={x + width / 2} y2={bottomWickY} stroke={color} strokeWidth={1} />
      <rect x={x + width * 0.2} y={bodyTop} width={width * 0.6} height={Math.max(1, bodyBottom - bodyTop)} fill={color} stroke={color} />
    </g>;
};
const generateBTCUSDMockData = (timeframe: string) => {
  const basePrice = 97634.69;
  const volatility = basePrice * 0.005;
  const points = timeframe.startsWith('M') ? 20 : timeframe.startsWith('H') ? 24 : timeframe === 'D' ? 30 : timeframe === 'M' ? 30 : 12;
  const timeInterval = timeframe === 'M1' ? 60000 : timeframe === 'M5' ? 300000 : timeframe === 'M15' ? 900000 : timeframe === 'H1' ? 3600000 : timeframe === 'H4' ? 14400000 : timeframe === 'D' ? 86400000 : timeframe === 'M' ? 2592000000 : 31536000000;
  return Array.from({
    length: points
  }, (_, i) => {
...
export const ForexChart = () => {
  ...
  useEffect(() => {
    const fetchCurrencyPairs = async () => {
      const {
        data,
        error
      } = await supabase.from('currency_pairs').select('symbol, display_name').eq('is_active', true);
      if (error) {
  ...
  useEffect(() => {
    const fetchBalanceAndMargin = async () => {
      ...
      const {
        data
      } = await supabase.from('user_balances').select('balance, equity, used_margin').eq('user_id', session.user.id).maybeSingle();
      if (data) {
  ...
  useEffect(() => {
    const fetchOpenPositions = async () => {
      ...
      const {
        data: positions
      } = await supabase.from('trades').select('*').eq('user_id', session.user.id).eq('status', 'open');
      if (positions) {
    ...
    fetchOpenPositions();
    const channel = supabase.channel('trading_updates').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'trades'
    }, fetchOpenPositions).subscribe();
    return () => {
      supabase.removeChannel(channel);
    };
  }, []);
  useEffect(() => {
    const fetchRealtimeData = async () => {
      ...
      const updatedPositions = openPositions.map(position => {
        const pnl = position.type === 'buy' ? (lastPrice - position.price) * position.amount * 100000 : (position.price - lastPrice) * position.amount * 100000;
        return {
          ...position,
          currentPnL: pnl
        };
  ...
  useEffect(() => {
    ...
    const prices = data.flatMap(d => [parseFloat(d.high), parseFloat(d.low), parseFloat(d.open), parseFloat(d.close)]);
  ...
  useEffect(() => {
    ...
    if (zoomDomain) {
      ...
      const visiblePrices = visibleData.flatMap(d => [parseFloat(d.high), parseFloat(d.low)]);
  ...
  const handleAddPair = async () => {
    const {
      error
    } = await supabase.from('currency_pairs').insert({
  ...
  const handleZoomIn = () => {
    ...
    if (!zoomDomain) {
      ...
      setZoomDomain([Math.max(0, center - Math.floor(newRange / 2)), Math.min(dataLength - 1, center + Math.floor(newRange / 2))]);
    }
  };
  const handleZoomOut = () => {
    const dataLength = data.length;
  ...
  const getBTCUSDMockData = (timeframe: string) => {
    const basePrice = selectedPair === 'BTCUSD' ? 97634.69 : 1.2000;
  ...
  useEffect(() => {
    const newData = selectedPair === 'BTCUSD' ? getBTCUSDMockData(timeframe) : generateMockData(timeframe);
    setData(newData);
          ...
          <Card className="w-full glass bg-zinc-50">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <Select value={selectedPair} onValueChange={setSelectedPair}>
                    ...
                    <SelectContent>
                      {currencyPairs.map(pair => <SelectItem key={pair.symbol} value={pair.symbol}>
                          {pair.display_name}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                  <ToggleGroup type="single" value={chartType} onValueChange={(value: 'candlestick' | 'line') => value && setChartType(value)}>
                    <ToggleGroupItem value="candlestick" className="mx-[10px] my-[10px] py-[10px] px-[10px] text-green-600">Candlestick</ToggleGroupItem>
            ...
            <CardContent className="mx-[10px] px-[10px] py-[10px] my-[10px] bg-zinc-50">
              <div className="h-[600px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={data}>
                    ...
                    <YAxis domain={[visiblePriceRange.min, visiblePriceRange.max]} tick={{
                    fill: "currentColor"
                  }} tickLine={{
                    stroke: "currentColor"
                  }} tickFormatter={value => selectedPair === 'BTCUSD' ? value.toFixed(0) : value.toFixed(4)} />
                    <Tooltip contentStyle={{
                    backgroundColor: "rgba(255, 255, 255, 0.9)",
                    border: "1px solid #ccc"
                  }} formatter={(value: any) => selectedPair === 'BTCUSD' ? parseFloat(value).toFixed(0) : parseFloat(value).toFixed(4)} />
                    {chartType === 'candlestick' ? <Bar dataKey="height" fill="none" barSize={12} shape={<CustomCandlestick />} data={data.map(d => ({
                    ...d,
                    height: 1,
                    ...
                    <Brush dataKey="time" height={30} stroke="#8884d8" startIndex={zoomDomain ? zoomDomain[0] : undefined} endIndex={zoomDomain ? zoomDomain[1] : undefined} onChange={domain => setZoomDomain([domain.startIndex, domain.endIndex])} />
                    <ReferenceLine y={currentPrice} stroke="#2563eb" strokeDasharray="3 3" label={{
                    value: 'Current Price',
                    position: 'right'
                  }} />
                    {openPositions.map((position, index) => <ReferenceLine key={position.id} y={position.price} stroke={position.type === 'buy' ? "#4ade80" : "#ef4444"} strokeDasharray="3 3" label={{
              ...
              <div className="mt-6 grid grid-cols-2 gap-4">
                <div className="flex items-center gap-4">
                  <Input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Amount" className="w-32" min="0.01" step="0.01" />
                  <span className="text-sm text-muted-foreground">
                    Lots (min: 0.01)
                  </span>
                </div>
                <div className="flex justify-end gap-4">
                  <Button variant="outline" className="w-24 bg-red-500/10 hover:bg-red-500/20 text-red-500" onClick={() => handleTrade('sell')} disabled={parseFloat(amount) * currentPrice < 2.50}>
                    Sell
                  </Button>
                  <Button variant="outline" className="w-24 bg-green-500/10 hover:bg-green-500/20 text-green-500" onClick={() => handleTrade('buy')} disabled={parseFloat(amount) * currentPrice < 2.50}>
                    Buy
                  </Button>
                </div>
              </div>
